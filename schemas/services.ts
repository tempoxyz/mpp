...
export const services: ServiceDef[] = [
  ...
  // ── WithZero ───────────────────────────────────────────────────────────
  {
    id: "withzero",
    name: "WithZero",
    url: "https://withzero.xyz",
    serviceUrl: "https://agents.withzero.xyz",
    description:
      "Create media, research companies and people, publish to the web, remember context, deploy apps, and schedule work—on demand, without vendor accounts or API keys.",
    categories: ["ai", "compute", "data", "media", "search", "social", "web"],
    integration: "third-party",
    tags: ["media", "research", "publishing", "automation", "memory"],
    status: "active",
    docs: {
      homepage: "https://withzero.xyz/",
      llmsTxt: "https://agents.withzero.xyz/llms.txt",
      apiReference: "https://withzero.xyz/openapi.json",
    },
    provider: { name: "WithZero", url: "https://withzero.xyz" },
    realm: "agents.withzero.xyz",
    intent: "charge",
    payments: [TEMPO_PAYMENT],
    endpoints: [
      {
        route: "POST /api/v1/annotate/reviews",
        desc: "Create an annotation review session. Pass `url` for a live webpage, or `zipData` (base64 zip) + optional `entryFile` for a static code bundle. Returns a `shareUrl` for the human reviewer and, for bundle reviews, a `bundleUrl` to download the original source afterward.",
        amount: "50000",
        unitType: "requests",
      },
      {
        route: "POST /api/v1/audio/songs/lyria-3-pro",
        desc: "Generate a complete, full-length song from a text prompt with Google's Lyria 3 Pro and return a hosted audio URL.",
        amount: "80000",
        unitType: "songs",
      },
      {
        route: "POST /api/v1/audio/speech/gemini-3.1-flash-tts",
        desc: "Convert text to natural-sounding speech with Google Gemini 3.1 Flash TTS and return a hosted audio URL.",
        dynamic: true,
        amountHint: "$0.01 per 250 characterss",
      },
      {
        route: "POST /api/v1/audio/speech/grok-voice",
        desc: "Convert text to natural-sounding speech with xAI's Grok Voice and return a hosted audio URL.",
        dynamic: true,
        amountHint: "$0.01 per 500 characterss",
      },
      {
        route: "POST /api/v1/audio/transcripts/gpt-4o-transcribe",
        desc: "Transcribe spoken audio to text with OpenAI's GPT-4o Transcribe.",
        dynamic: true,
        amountHint: "$0.01 per 2 minutess",
      },
      {
        route: "POST /api/v1/audio/transcripts/whisper-large-v3",
        desc: "Transcribe spoken audio to text with OpenAI Whisper Large v3.",
        dynamic: true,
        amountHint: "$0.01 per 5 minutess",
      },
      {
        route: "POST /api/v1/crustdata/company/enrich",
        desc: "Core workflow route: hydrate one account with detailed firmographic company profile data after identify or company search. Defaults to basic_info, headcount, and people so account enriches surface leadership and decision-maker data when Crustdata has it. Default pricing is $0.10 per Crustdata credit with a $0.01 minimum successful-call settlement. Upstream Crustdata errors settle $0. DB company enrich is priced at two Crustdata credits per identifier.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/crustdata/company/identify",
        desc: "Core workflow route: normalize company inputs from domains, names, professional-network company URLs, or Crustdata IDs before account enrichment or people search. Minimum-charge route: $0.01 on successful upstream calls.",
        amount: "10000",
        unitType: "cents",
      },
      {
        route: "POST /api/v1/crustdata/company/search",
        desc: "Core workflow route: build a target account page from the cached Crustdata company dataset using provider-native filters, fields, sorts, cursor, aggregations, and limit. Default pricing is $0.10 per Crustdata credit with a $0.01 minimum successful-call settlement. Upstream Crustdata errors settle $0. CompanyDB search is priced at 0.03 Crustdata credits per returned company.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/crustdata/company/search/autocomplete",
        desc: "Query-builder helper route: autocomplete company search field values for UI/filter construction. Minimum-charge route: $0.01 on successful upstream calls.",
        amount: "10000",
        unitType: "cents",
      },
      {
        route: "POST /api/v1/crustdata/job/search",
        desc: "Core workflow route: search cached job listings to infer account growth, hiring priorities, expansion, or technology adoption. Default pricing is $0.10 per Crustdata credit with a $0.01 minimum successful-call settlement. Upstream Crustdata errors settle $0. Indexed job search is priced at 0.03 Crustdata credits per returned job.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/crustdata/job/search/autocomplete",
        desc: "Query-builder helper route: autocomplete job search field values for hiring-signal filter construction. Minimum-charge route: $0.01 on successful upstream calls.",
        amount: "10000",
        unitType: "cents",
      },
      {
        route: "POST /api/v1/crustdata/person/contact/enrich",
        desc: "Core workflow route: enrich one person's contact records by professional-network profile URL or business email. Use this when the workflow specifically needs business emails, personal emails, or phone numbers; request only the contact sub-fields needed to control credits. Default pricing is $0.10 per Crustdata credit with a $0.01 minimum successful-call settlement. Upstream Crustdata errors settle $0. Contact enrich is priced at one to five Crustdata credits per record and may require contact-enrich entitlement on the configured Crustdata key.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/crustdata/person/enrich",
        desc: "Core workflow route: enrich one person by professional-network profile URL or business email. Default pricing is $0.10 per Crustdata credit with a $0.01 minimum successful-call settlement. Upstream Crustdata errors settle $0. DB profile enrich starts at one credit by profile URL or two credits by business-email reverse lookup; contact and developer field bundles settle according to Crustdata's documented add-ons up to seven credits per profile.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/crustdata/person/search",
        desc: "Core workflow route: find a page of people and prospects from the cached Crustdata person dataset using provider-native filters, fields, sorts, cursor, aggregations, and limit. Default pricing is $0.10 per Crustdata credit with a $0.01 minimum successful-call settlement. Upstream Crustdata errors settle $0. PersonDB search is priced at 0.03 Crustdata credits per returned profile.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/crustdata/person/search/autocomplete",
        desc: "Query-builder helper route: autocomplete person search field values for UI/filter construction. Minimum-charge route: $0.01 on successful upstream calls.",
        amount: "10000",
        unitType: "cents",
      },
      {
        route: "POST /api/v1/crustdata/person/search/role-cluster",
        desc: "Decision-maker helper route: find founders, executives, GTM, product, technical, partnerships, or other role-cluster candidates at one current employer using Crustdata person/search nested filters. Returns raw Crustdata people profiles with basic_profile, social_handles, and experience rows by default so callers can inspect profile URLs and matching current employment. Default pricing is $0.10 per Crustdata credit with a $0.01 minimum successful-call settlement. Upstream Crustdata errors settle $0. Priced like PersonDB search at 0.03 Crustdata credits per returned profile.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/crustdata/web/enrich/live",
        desc: "Core workflow route: fetch and enrich public web pages returned by web search or supplied by the caller. Default pricing is $0.10 per Crustdata credit with a $0.01 minimum successful-call settlement. Upstream Crustdata errors settle $0. Estimates one Crustdata credit per URL.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/crustdata/web/search/live",
        desc: "Core workflow route: run live web, news, scholar, AI, or social search to add external context around accounts, people, funding, markets, or recent events. Default pricing is $0.10 per Crustdata credit with a $0.01 minimum successful-call settlement. Upstream Crustdata errors settle $0. Estimates one Crustdata credit per query.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/deploy/apps",
        desc: "Node.js hosting: ship the Node app you built locally, unchanged — full server runtime on Cloud Run with a dedicated Postgres. Native modules OK. App reads DATABASE_URL + listens on PORT. Filesystem is writable but ephemeral (scale-to-zero) — keep durable state in Postgres.",
        amount: "600000",
        unitType: "terms",
      },
      {
        route: "POST /api/v1/image/images/gpt-5-image",
        desc: "Generate images from a text prompt with OpenAI GPT-5 Image — strongest instruction following and legible in-image text. Returns hosted image URLs.",
        amount: "210000",
        unitType: "images",
      },
      {
        route: "POST /api/v1/image/images/grok-imagine",
        desc: "Generate images from a text prompt with xAI's Grok Imagine quality tier. Returns hosted image URLs.",
        amount: "50000",
        unitType: "images",
      },
      {
        route: "POST /api/v1/image/images/nano-banana-2",
        desc: "Generate images from a text prompt with Google's Nano Banana 2 (Gemini 3.1 Flash Image) — fast, high-quality generation. Returns hosted image URLs.",
        amount: "70000",
        unitType: "images",
      },
      {
        route: "POST /api/v1/image/images/nano-banana-pro",
        desc: "Generate images from a text prompt with Google's Nano Banana Pro (Gemini 3 Pro Image) — highest-quality photorealistic output. Returns hosted image URLs.",
        amount: "140000",
        unitType: "images",
      },
      {
        route: "POST /api/v1/memory/memories",
        desc: "Remember a fact or context in durable agent memory.",
        amount: "10000",
        unitType: "ops",
      },
      {
        route: "POST /api/v1/openrouter/chat/completions",
        desc: "OpenAI-compatible chat completion via any OpenRouter model, paid in one shot: price = estimated input tokens x input rate + max_tokens x output rate, resolved before the call and capped per model (from $0.10 for cheap models up to $5 for premium models like anthropic/claude-fable-5). For longer generations use POST /api/v1/chat/completions/metered. 📖 Full guide: https://openrouter.withzero.xyz/llms.txt",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/openrouter/chat/completions/metered",
        desc: "OpenAI-compatible chat completion via any OpenRouter model with metered at-cost billing: escrow a worst-case reservation (up to 5 USDC), pay only actual token usage — the difference is refunded after the call. Use this route for long generations, big contexts, and reasoning models; the fixed route caps out much lower. 📖 Full guide: https://openrouter.withzero.xyz/llms.txt",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/pipe0/company/identity",
        desc: "Resolve one company name through Pipe0 company:identity@3. At-cost fixed cap: max($0.01, 0.30 Pipe0 credits × $999/34,000 credits). Useful as a lightweight first step, fallback, or corroboration step before Crustdata enrichment workflows.",
        amount: "10000",
        unitType: "cents",
      },
      {
        route: "POST /api/v1/pipe0/company/overview",
        desc: "Enrich one company domain or professional-network company profile URL through Pipe0 company:overview@3. At-cost cap: 1.00 Pipe0 credit × $999/34,000 credits for company description, industry, geography, size, and related overview fields.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/pipe0/company/search",
        desc: "Search one Pipe0 company profile page through companies:profiles:crustdata@2 with provider-native include/exclude filters, cursor, output fields, and a bounded 100-result limit. At-cost cap: 2 Pipe0 credits per page × $999/34,000 credits.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/pipe0/person/profile",
        desc: "Enrich one person profile URL through Pipe0 people:profile:waterfall@1. At-cost cap: 1.00 Pipe0 credit × $999/34,000 credits for successful profile enrichment.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/pipe0/pipes/run/sync",
        desc: "Composable Pipe0-native workflow pass-through for allowlisted pipe IDs only. Runs POST /v1/pipes/run/sync for workflows that are not exposed directly by Crustdata or Wiza. Settles at cost from the bounded pipe/input estimate because Pipe0 does not return actual credits charged.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/pipe0/search/run/sync",
        desc: "Composable Pipe0-native search workflow pass-through for allowlisted singular search IDs only. Runs POST /v1/search/run/sync for safe allowlisted company-profile search and settles at the allowlisted per-page Pipe0 catalog cost.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/pixelart/canvas",
        desc: "Create a new pixel art canvas. Returns editing, manifest, full-spec, guide, and interactive-view URLs. Optionally pre-load base64 RGBA pixels. Full agent guide: https://pixelart.withzero.xyz/llms.txt",
        amount: "10000",
        unitType: "requests",
      },
      {
        route: "POST /api/v1/pixelart/canvas/:id/animate",
        desc: "Generate a flattened animation cycle through Retro Diffusion and atomically replace the canvas frames. This is the legacy external-provider compatibility route; use segmentation plus layer transforms when authored, deterministic motion is required. Full agent guide: https://pixelart.withzero.xyz/llms.txt",
        amount: "300000",
        unitType: "requests",
      },
      {
        route: "GET /api/v1/pixelart/canvas/:id/export.gif",
        desc: "Export the composited animation as GIF89a bytes. Query overrides: fps=1..60; scale=1..8 (default 4); loop=true|false; transparent=true|false (default true). Stored FPS and loop are used when omitted. Full agent guide: https://pixelart.withzero.xyz/llms.txt",
        amount: "20000",
        unitType: "requests",
      },
      {
        route: "POST /api/v1/pixelart/canvas/:id/segment/semantic",
        desc: "Group deterministic regions into named animation parts such as head, body, cape, limbs, and held items. Dry-run by default; materialization is revision-guarded and replaces only the selected frame by default. Full agent guide: https://pixelart.withzero.xyz/llms.txt",
        amount: "20000",
        unitType: "requests",
      },
      {
        route: "POST /api/v1/pixelart/canvas/:id/sprite-sheet-releases",
        desc: "Publish a content-addressed PNG sprite sheet and deterministic JSON engine manifest. Repeating the same source and layout returns the same immutable release URLs. Full agent guide: https://pixelart.withzero.xyz/llms.txt",
        amount: "20000",
        unitType: "requests",
      },
      {
        route: "POST /api/v1/pixelart/canvas/import",
        desc: "Convert a public PNG/JPEG/WebP/GIF or base64 image into an editable, palette-quantized pixel canvas. Full agent guide: https://pixelart.withzero.xyz/llms.txt",
        amount: "10000",
        unitType: "requests",
      },
      {
        route: "POST /api/v1/video/videos/kling-v3.0-pro",
        desc: "Generate a video from a text prompt with Kuaishou Kling 3.0 Pro — premium realism and motion, clips from 3 to 15 seconds. Returns a job id; poll the status route for the finished video URL.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/video/videos/sora-2-pro",
        desc: "Generate a video from a text prompt with OpenAI Sora 2 Pro — production-quality motion with synchronized audio. Returns a job id; poll the status route for the finished video URL.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/video/videos/veo-3.1",
        desc: "Generate a video from a text prompt with Google Veo 3.1 — maximum visual fidelity up to 4K with native audio. Returns a job id; poll the status route for the finished video URL.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/video/videos/veo-3.1-fast",
        desc: "Generate a video from a text prompt with Google Veo 3.1 Fast — high quality with native audio at a faster turnaround. Returns a job id; poll the status route for the finished video URL.",
        dynamic: true,
        amountHint: "$0.01 per cents",
      },
      {
        route: "POST /api/v1/wiza/company-enrichments",
        desc: "Enrich one company by name, domain, LinkedIn ID, or LinkedIn slug. Returns Wiza company industry, size, revenue, funding, social, and location fields. Metered billing settles from Wiza company enrichment API credits.",
        dynamic: true,
        amountHint: "$0.01 per credits",
      },
      {
        route: "POST /api/v1/wiza/individual-reveals",
        desc: "Start a Wiza individual reveal for one contact and poll until completion. Accepts name + company/domain, email, or LinkedIn profile URL. Advertised unit price starts at $0.01/profile; final metered settlement follows Wiza api_credits.total and can be up to $0.08/profile when Wiza returns contact-field credits.",
        dynamic: true,
        amountHint: "$0.01 per credits",
      },
      {
        route: "POST /api/v1/wiza/lists",
        desc: "Create a Wiza enrichment list from explicit contacts, emails, or LinkedIn profile URLs. Supports up to 30 items per call; payment escrow is sized from the submitted item count and final settlement comes from Wiza list api_credits.total.",
        dynamic: true,
        amountHint: "$0.01 per credits",
      },
      {
        route: "POST /api/v1/wiza/prospect-lists",
        desc: "Create an enriched Wiza prospect list from search filters, including company-name aliases normalized to Wiza job_company. Capped at 30 profiles per call; payment escrow is sized from list.max_profiles and final settlement comes from Wiza list api_credits.total.",
        dynamic: true,
        amountHint: "$0.01 per credits",
      },
      {
        route: "POST /api/v1/wiza/prospect-lists/continue",
        desc: "Continue a Wiza prospect list search to fetch additional matching profiles. Capped at 30 additional profiles per call; payment escrow is sized from max_profiles when provided and final settlement comes from Wiza list api_credits.total.",
        dynamic: true,
        amountHint: "$0.01 per credits",
      },
      {
        route: "POST /api/v1/zeroman/credits",
        desc: "Insert a coin: pay to mint an arcade credit (3 lives) for a run of ZEROMAN.",
        amount: "100000",
        unitType: "requests",
      },
    ],
