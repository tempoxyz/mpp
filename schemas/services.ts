...
export const services: ServiceDef[] = [
  ...
  // ── StableEmail ────────────────────────────────────────────────────────
  {
    ...
    payment: TEMPO_PAYMENT,
    endpoints: [
      { route: "POST /api/send", desc: "Send email from relay address", amount: "20000" },
      { route: "POST /api/subdomain/buy", desc: "Purchase a custom email subdomain", amount: "5000000" },
      { route: "POST /api/subdomain/send", desc: "Send email from custom subdomain", amount: "5000" },
      { route: "POST /api/subdomain/signers", desc: "Add or remove authorized wallet signers" },
      { route: "POST /api/subdomain/update", desc: "Update subdomain settings (catch-all forwarding)" },
      { route: "GET /api/subdomain/status", desc: "Check DNS/SES verification status" },
      { route: "POST /api/subdomain/inbox/create", desc: "Create inbox on subdomain", amount: "250000" },
      { route: "POST /api/subdomain/inbox/list", desc: "List subdomain inboxes" },
      { route: "POST /api/subdomain/inbox/update", desc: "Update subdomain inbox settings" },
      { route: "POST /api/subdomain/inbox/delete", desc: "Delete inbox from subdomain" },
      { route: "POST /api/subdomain/inbox/messages", desc: "List subdomain inbox messages", amount: "1000" },
      { route: "POST /api/subdomain/inbox/messages/read", desc: "Read a subdomain inbox message", amount: "1000" },
      { route: "POST /api/subdomain/inbox/messages/delete", desc: "Delete a subdomain inbox message" },
      { route: "POST /api/inbox/buy", desc: "Buy a forwarding inbox (30 days)", amount: "1000000" },
      { route: "POST /api/inbox/topup", desc: "Extend inbox 30 days", amount: "1000000" },
      { route: "POST /api/inbox/topup/quarter", desc: "Extend inbox 90 days (save 17%)", amount: "2500000" },
      { route: "POST /api/inbox/topup/year", desc: "Extend inbox 365 days (save 34%)", amount: "8000000" },
      { route: "POST /api/inbox/send", desc: "Send email from inbox address", amount: "5000" },
      { route: "GET /api/inbox/status", desc: "Check inbox status and expiry" },
      { route: "POST /api/inbox/update", desc: "Update inbox settings" },
      { route: "POST /api/inbox/cancel", desc: "Cancel inbox and get pro-rata refund" },
      { route: "POST /api/inbox/messages", desc: "List messages in inbox", amount: "1000" },
      { route: "POST /api/inbox/messages/read", desc: "Read a single inbox message", amount: "1000" },
      { route: "POST /api/inbox/messages/delete", desc: "Delete an inbox message" },
    ],
  },
  ...
  // ── StableEnrich ───────────────────────────────────────────────────────
  {
    ...
    integration: "first-party",
    tags: ["apollo", "exa", "firecrawl", "google-maps", "grok", "linkedin", "reddit", "enrichment", "research"],
    docs: {
    ...
    endpoints: [
      // Apollo
      { route: "POST /api/apollo/people-search", desc: "Find prospects by filters", amount: "20000" },
      { route: "POST /api/apollo/people-enrich", desc: "Enrich single person by email/name/domain", amount: "49500" },
      { route: "POST /api/apollo/org-search", desc: "Find companies by filters", amount: "20000" },
      { route: "POST /api/apollo/org-enrich", desc: "Enrich single company by domain", amount: "49500" },
      // Clado
      { route: "POST /api/clado/linkedin-scrape", desc: "Scrape full LinkedIn profile data", amount: "40000" },
      { route: "POST /api/clado/contacts-enrich", desc: "Enrich contact info from LinkedIn URL, email, or phone", amount: "200000" },
      // Exa
      { route: "POST /api/exa/search", desc: "Neural web search", amount: "10000" },
      { route: "POST /api/exa/find-similar", desc: "Find pages similar to a URL", amount: "10000" },
      { route: "POST /api/exa/contents", desc: "Extract content from URLs", amount: "2000" },
      { route: "POST /api/exa/answer", desc: "AI-generated answers with citations", amount: "10000" },
      // Firecrawl
      { route: "POST /api/firecrawl/scrape", desc: "Scrape a URL with full JavaScript rendering", amount: "12600" },
      { route: "POST /api/firecrawl/search", desc: "Search the web and get scraped results", amount: "25200" },
      // Google Maps
      { route: "POST /api/google-maps/text-search/full", desc: "Text search with full details", amount: "80000" },
      { route: "POST /api/google-maps/text-search/partial", desc: "Text search with basic details", amount: "20000" },
      { route: "POST /api/google-maps/nearby-search/full", desc: "Nearby search with full details", amount: "80000" },
      { route: "POST /api/google-maps/nearby-search/partial", desc: "Nearby search with basic details", amount: "20000" },
      { route: "GET /api/google-maps/place-details/full", desc: "Full place details by ID", amount: "50000" },
      { route: "GET /api/google-maps/place-details/partial", desc: "Partial place details by ID", amount: "20000" },
      // Grok (X/Twitter)
      { route: "POST /api/grok/x-search", desc: "Search X/Twitter posts", amount: "20000" },
      { route: "POST /api/grok/user-search", desc: "Search X/Twitter users", amount: "20000" },
      { route: "POST /api/grok/user-posts", desc: "Get recent posts from an X user", amount: "20000" },
      // Serper
      { route: "POST /api/serper/news", desc: "Google News search", amount: "40000" },
      { route: "POST /api/serper/shopping", desc: "Google Shopping search", amount: "40000" },
      // Reddit
      { route: "POST /api/reddit/search", desc: "Search Reddit posts", amount: "20000" },
      { route: "POST /api/reddit/post-comments", desc: "Get post details and comments", amount: "20000" },
      // Whitepages
      { route: "POST /api/whitepages/person-search", desc: "Search for people by name, phone, or address", amount: "440000" },
      { route: "POST /api/whitepages/property-search", desc: "Property ownership and resident details by address", amount: "440000" },
      // Hunter
      { route: "POST /api/hunter/email-verifier", desc: "Verify email deliverability", amount: "30000" },
      // Influencer
      { route: "POST /api/influencer/enrich-by-email", desc: "Find social profiles by email", amount: "400000" },
      { route: "POST /api/influencer/enrich-by-social", desc: "Enrich social media profile with contact info", amount: "400000" },
    ],
  },
  ...
  // ── StablePhone ────────────────────────────────────────────────────────
  {
    ...
    payment: TEMPO_PAYMENT,
    endpoints: [
      { route: "POST /api/call", desc: "Make an AI phone call", amount: "540000" },
      { route: "GET /api/call/:id", desc: "Get call status and transcript" },
      { route: "POST /api/number", desc: "Buy a phone number (30 days)", amount: "20000000" },
      { route: "POST /api/number/topup", desc: "Extend a phone number 30 days", amount: "15000000" },
      { route: "GET /api/numbers", desc: "List your phone numbers" },
      { route: "POST /api/lookup", desc: "iMessage/FaceTime lookup", amount: "50000" },
  ...
  // ── StableSocial ───────────────────────────────────────────────────────
  {
    ...
    categories: ["social", "data"],
    integration: "first-party",
    tags: ["tiktok", "instagram", "twitter", "facebook", "reddit", "scraping", "social"],
    docs: {
    ...
    endpoints: [
      // TikTok
      { route: "POST /api/tiktok/profile", desc: "Get TikTok user profile", amount: "60000" },
      { route: "POST /api/tiktok/posts", desc: "Get TikTok user posts", amount: "60000" },
      { route: "POST /api/tiktok/post-comments", desc: "Get TikTok video comments", amount: "60000" },
      { route: "POST /api/tiktok/comment-replies", desc: "Get TikTok comment replies", amount: "60000" },
      { route: "POST /api/tiktok/followers", desc: "Get TikTok followers", amount: "60000" },
      { route: "POST /api/tiktok/following", desc: "Get TikTok following", amount: "60000" },
      { route: "POST /api/tiktok/search", desc: "Search TikTok posts by keyword", amount: "60000" },
      { route: "POST /api/tiktok/search-hashtag", desc: "Search TikTok by hashtag", amount: "60000" },
      { route: "POST /api/tiktok/search-profiles", desc: "Search TikTok user profiles", amount: "60000" },
      { route: "POST /api/tiktok/search-music", desc: "Search TikTok posts by sound", amount: "60000" },
      // Instagram
      { route: "POST /api/instagram/profile", desc: "Get Instagram user profile", amount: "60000" },
      { route: "POST /api/instagram/posts", desc: "Get Instagram user posts", amount: "60000" },
      { route: "POST /api/instagram/post-comments", desc: "Get Instagram post comments", amount: "60000" },
      { route: "POST /api/instagram/comment-replies", desc: "Get Instagram comment replies", amount: "60000" },
      { route: "POST /api/instagram/followers", desc: "Get Instagram followers", amount: "60000" },
      { route: "POST /api/instagram/following", desc: "Get Instagram following", amount: "60000" },
      { route: "POST /api/instagram/stories", desc: "Get Instagram user stories", amount: "60000" },
      { route: "POST /api/instagram/highlights", desc: "Get Instagram user highlights", amount: "60000" },
      { route: "POST /api/instagram/search", desc: "Search Instagram posts by keyword", amount: "60000" },
      { route: "POST /api/instagram/search-tags", desc: "Search Instagram by tag", amount: "60000" },
      // X/Twitter
      { route: "POST /api/x/profile", desc: "Get X/Twitter user profile", amount: "60000" },
      { route: "POST /api/x/posts", desc: "Get X/Twitter user posts", amount: "60000" },
      { route: "POST /api/x/post-replies", desc: "Get X/Twitter post replies", amount: "60000" },
      { route: "POST /api/x/post-retweets", desc: "Get X/Twitter retweet profiles", amount: "60000" },
      { route: "POST /api/x/post-quotes", desc: "Get X/Twitter quote tweets", amount: "60000" },
      { route: "POST /api/x/followers", desc: "Get X/Twitter followers", amount: "60000" },
      { route: "POST /api/x/following", desc: "Get X/Twitter following", amount: "60000" },
      { route: "POST /api/x/search", desc: "Search X/Twitter posts by keyword", amount: "60000" },
      { route: "POST /api/x/search-profiles", desc: "Search X/Twitter user profiles", amount: "60000" },
      // Facebook
      { route: "POST /api/facebook/profile", desc: "Get Facebook page/user profile", amount: "60000" },
      { route: "POST /api/facebook/posts", desc: "Get Facebook page/user posts", amount: "60000" },
      { route: "POST /api/facebook/post-comments", desc: "Get Facebook post comments", amount: "60000" },
      { route: "POST /api/facebook/comment-replies", desc: "Get Facebook comment replies", amount: "60000" },
      { route: "POST /api/facebook/followers", desc: "Get Facebook followers", amount: "60000" },
      { route: "POST /api/facebook/following", desc: "Get Facebook following", amount: "60000" },
      { route: "POST /api/facebook/search", desc: "Search Facebook posts by keyword", amount: "60000" },
      { route: "POST /api/facebook/search-people", desc: "Search Facebook people profiles", amount: "60000" },
      { route: "POST /api/facebook/search-pages", desc: "Search Facebook page profiles", amount: "60000" },
      { route: "POST /api/facebook/search-groups", desc: "Search Facebook group profiles", amount: "60000" },
      // Reddit
      { route: "POST /api/reddit/post", desc: "Get Reddit post details", amount: "60000" },
      { route: "POST /api/reddit/post-comments", desc: "Get Reddit post comments", amount: "60000" },
      { route: "POST /api/reddit/comment", desc: "Get Reddit comment details", amount: "60000" },
      { route: "POST /api/reddit/search", desc: "Search Reddit posts by keyword", amount: "60000" },
      { route: "POST /api/reddit/search-profiles", desc: "Search Reddit user profiles", amount: "60000" },
      { route: "POST /api/reddit/subreddit", desc: "Get subreddit posts", amount: "60000" },
      // Polling
  ...
  // ── StableStudio ───────────────────────────────────────────────────────
  {
    ...
    categories: ["ai", "media"],
    integration: "first-party",
    tags: ["image", "video", "generation", "nano-banana", "gpt-image", "grok", "flux", "sora", "veo", "seedance", "wan"],
    docs: {
    ...
    endpoints: [
      // Nano Banana
      { route: "POST /api/generate/nano-banana/generate", desc: "Nano Banana image generation", amount: "39000" },
      { route: "POST /api/generate/nano-banana/edit", desc: "Nano Banana image editing", amount: "39000" },
      { route: "POST /api/generate/nano-banana-pro/generate", desc: "Nano Banana Pro image generation (up to 4K)", dynamic: true },
      { route: "POST /api/generate/nano-banana-pro/edit", desc: "Nano Banana Pro image editing (up to 4K)", dynamic: true },
      // Grok
      { route: "POST /api/generate/grok/generate", desc: "Grok image generation", amount: "70000" },
      { route: "POST /api/generate/grok/edit", desc: "Grok image editing", amount: "22000" },
      { route: "POST /api/generate/grok-video/generate", desc: "Grok video generation", dynamic: true },
      // GPT Image
      { route: "POST /api/generate/gpt-image-1/generate", desc: "GPT Image 1 generation", dynamic: true },
      { route: "POST /api/generate/gpt-image-1/edit", desc: "GPT Image 1 editing", dynamic: true },
      { route: "POST /api/generate/gpt-image-1.5/generate", desc: "GPT Image 1.5 generation", dynamic: true },
      { route: "POST /api/generate/gpt-image-1.5/edit", desc: "GPT Image 1.5 editing", dynamic: true },
      // Flux
      { route: "POST /api/generate/flux-2-pro/generate", desc: "Flux 2 Pro image generation", dynamic: true },
      { route: "POST /api/generate/flux-2-pro/edit", desc: "Flux 2 Pro image editing", dynamic: true },
      // Seedance
      { route: "POST /api/generate/seedance/t2v", desc: "Seedance text-to-video", dynamic: true },
      { route: "POST /api/generate/seedance/i2v", desc: "Seedance image-to-video", dynamic: true },
      { route: "POST /api/generate/seedance-fast/t2v", desc: "Seedance Fast text-to-video", dynamic: true },
      { route: "POST /api/generate/seedance-fast/i2v", desc: "Seedance Fast image-to-video", dynamic: true },
      // Wan
      { route: "POST /api/generate/wan-2.6/t2v", desc: "Wan 2.6 text-to-video", dynamic: true },
      { route: "POST /api/generate/wan-2.6/i2v", desc: "Wan 2.6 image-to-video", dynamic: true },
      // Sora
      { route: "POST /api/generate/sora-2/generate", desc: "Sora 2 video generation", dynamic: true },
      { route: "POST /api/generate/sora-2-pro/generate", desc: "Sora 2 Pro video generation", dynamic: true },
      // Veo
      { route: "POST /api/generate/veo-3.1/generate", desc: "Veo 3.1 video generation", dynamic: true },
      { route: "POST /api/generate/veo-3.1-fast/generate", desc: "Veo 3.1 Fast video generation", dynamic: true },
      // Upload & Jobs
      { route: "POST /api/upload", desc: "Upload image for editing or image-to-video", amount: "10000" },
      { route: "POST /api/upload/confirm", desc: "Confirm uploaded file" },
      { route: "GET /api/jobs/:jobId", desc: "Poll job status and retrieve results" },
  ...
  // ── StableUpload ───────────────────────────────────────────────────────
  {
    ...
    payment: TEMPO_PAYMENT,
    endpoints: [
      { route: "POST /api/upload", desc: "Buy an upload slot (10MB $0.02, 100MB $0.20, 1GB $2.00)", dynamic: true },
      { route: "GET /api/uploads", desc: "List uploads for authenticated wallet" },
      { route: "GET /api/download/:uploadId", desc: "Get upload details by ID" },
      { route: "POST /api/site", desc: "Buy a site upload slot for zip hosting", dynamic: true },
      { route: "POST /api/site/activate", desc: "Extract zip and make site live" },
      { route: "POST /api/site/domain", desc: "Connect a custom domain to a site" },
      { route: "GET /api/site/domain/status", desc: "Check TLS provisioning status" },
    ],
  },
];
