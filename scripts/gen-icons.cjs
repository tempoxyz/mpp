...
function svg(viewBox, paths) {
  ...
  const iconW = vbW * scale;
  const iconH = vbH * scale;
  const tx = (76.8 + (inner - iconW) / 2 - minX * scale).toFixed(2);
  const ty = (76.8 + (inner - iconH) / 2 - minY * scale).toFixed(2);
  const s = scale.toFixed(6);

  const arr = Array.isArray(paths) ? paths : [paths];
  const d = arr.map((p) => `<path d="${p}"/>`).join("");

  return [
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="512" height="512">`,
    `<rect fill="#2A2A2A" width="512" height="512" rx="64"/>`,
    `<g fill="#E8E8EC" transform="translate(${tx} ${ty}) scale(${s})">`,
    d,
    `</g></svg>`,
  ].join("");
}
...
const icons = {
  ...
  elevenlabs: ["0 0 24 24", "M4.6035 0v24h4.9317V0zm9.8613 0v24h4.9317V0z"],
  exa: [
    "0 0 24 24",
    "M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z",
  ],
  fal: ["0 0 24 24", "M13 2L3 14h7l-2 8 10-12h-7l2-8z"],
  firecrawl: [
    "0 0 24 24",
...
const gmapsServices = [
  ...
  "googlemaps-places-v2",
  "googlemaps-pollen",
  "googlemaps-roads",
  "googlemaps-routes",
  "googlemaps-solar",
  "googlemaps-tiles",
  "googlemaps-validation",
  "googlemaps-weather",
];

// Override map for services whose first letter isn't a good icon
...
async function main() {
  ...
  // --- Strict mode: fail if any non-allowed letter fallbacks exist ---
  const letterAllowlist = new Set([]);
  const strictViolations = needsIcon.filter((id) => !letterAllowlist.has(id));
  if (strict && strictViolations.length > 0) {
    console.error(
      `\n--strict: ${strictViolations.length} services have letter-fallback icons: ${strictViolations.join(", ")}`,
    );
    console.error(
