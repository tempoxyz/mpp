...
See [MPP vs x402](/mpp-vs-x402) for a full side-by-side comparison, or [Running x402 servers with mppx](/guides/upgrade-x402#running-x402-servers-with-mppx) for an implementation guide.
...
Yes. The core x402 "exact" flows map directly onto MPP's charge intent. `mppx` can serve x402 and MPP clients from the same route. See [Running x402 servers with mppx](/guides/upgrade-x402#running-x402-servers-with-mppx).
