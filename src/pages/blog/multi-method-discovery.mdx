...
<p className="blog-date" style={{ color: 'var(--vocs-color_text3)', fontSize: '14px' }}>Apr 27, 2026</p>

# Multi-method discovery [Advertise multiple payment methods in one discovery document]

One of the core tenants of MPP's design is that it is payment agnostic and flexible. Over ten payment methods are live across the protocol, spanning stablecoins on every EVM network, cards, and even Bitcoin. As more methods have been added to the protocol, we have seen an increase in services accepting multiple payment methods, multiple currencies, and multiple intent types on the same endpoint.
