...
<p className="blog-date" style={{ color: 'var(--vocs-color_text3)', fontSize: '14px' }}>Friday, June 12, 2026</p>

# Improved sessions [Faster, cheaper billing and formally specified]
...
- **Automatic settlement** lets the server settle accepted usage in the background. A server-owned `settlementSchedule` triggers settlement by spend amount, metered units, or elapsed time, so the hot path stays off-chain while usage accumulates—no manual settlement job required.
- **Session resumption** lets a client reuse an existing channel across restarts. Pass a `sessionStore` to persist channel hints, and the next `tempo.session.manager()` resumes from the stored channel and server snapshot instead of opening and funding a new one.
...
| Operation | Legacy Sessions | Improved sessions | Change |
|---|---:|---:|---:|
