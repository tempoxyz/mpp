...
:::info[Why `proof` matters]
This removes a griefing vector in identity-only flows. A server can no longer make an end user burn about `$0.001` in network fees just to prove key ownership when the end user is the fee payer.
:::
