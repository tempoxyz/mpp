...
The core specification is published at [paymentauth.org](https://paymentauth.org). The [Payment HTTP Authentication Scheme](https://datatracker.ietf.org/doc/draft-httpauth-payment/) is also an active individual IETF Internet-Draft with intended status Standards Track.

As an Internet-Draft, the specification is governed by IETF Trust licensing under [BCP 78](https://www.rfc-editor.org/info/bcp78) and [BCP 79](https://www.rfc-editor.org/info/bcp79). Code components within the document are licensed under the Revised BSD License per the IETF Trust Legal Provisions. The submission makes the core available for open review through the IETF process.
