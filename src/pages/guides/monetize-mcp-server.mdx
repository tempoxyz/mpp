...
const result = await mppx.compose(
  ['tempo/charge', { amount: '0.01', currency: pathUSD }],
  ['tempo/charge', { amount: '0.01', currency: USDCe }],
)(extra)
...
Every composed method must use the configured MCP transport. HTTP-only `canOffer` and `selectOffers` policies don't run for MCP composition.
