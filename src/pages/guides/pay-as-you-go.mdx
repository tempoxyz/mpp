...
```txt
Use https://mpp.sh/guides/pay-as-you-go.md as reference.
...
Next, we will create the gallery route. This route is **currently unpaid**.
...
Next, we will add payment verification using `mppx.session` as route middleware.
...
Next, we will add payment verification using `mppx.session`. If the status is `402`, return the Challenge. Otherwise, fetch the photo and attach a Receipt to the response.
