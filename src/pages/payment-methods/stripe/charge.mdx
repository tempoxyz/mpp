...
Pass a resolver when the options depend on the verified Credential or canonical request. The resolver runs after non-mutating method validation, when available, and before `mppx` creates an SPT `PaymentIntent` or broadcasts a stablecoin payment. It doesn't run for the initial Challenge or an invalid Credential.

```ts [server.ts]
declare function createTaxCalculation(amount: unknown): Promise<string>
...
const result = await mppx.charge({
  ...
  paymentIntentOptions: async ({ challenge, request }) => ({
    hooks: {
      inputs: {
        tax: { calculation: await createTaxCalculation(request.amount) },
      },
    },
...
The resolver receives `challenge`, `credential`, optional verified `envelope`, and canonical `request` fields. It can return options or a Promise of options. Resolver errors stop the terminal payment operation.
