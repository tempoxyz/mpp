...
Set `html` on the method to render a Stripe Elements payment form when a browser navigates to the endpoint. The page collects card details, creates a `PaymentMethod`, mints an SPT through your `createTokenUrl`, and submits the Credential via a Service Worker.
...
See the [rich payment pages guide](/guides/payment-links) for a full walkthrough and live demo.
