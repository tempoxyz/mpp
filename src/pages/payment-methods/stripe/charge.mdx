...
Pass `metadata` to `stripe.create()` to attach key-value pairs to `PaymentIntent` objects created or recorded by its SPT and stablecoin charge methods. Pass `metadata` to an individual `payments.base.charge()` or `payments.tempo.charge()` call to add or override keys for that method. For a standalone SPT method, pass `metadata` to `stripe.spt()`.

`mppx` always adds `machine_payment: 'true'` so you can identify machine payments in Stripe. This reserved key overrides a matching key in your metadata.

For per-request metadata, pass `paymentIntentOptions.metadata` with the route options. Request-scoped keys take precedence over integration-level and per-method metadata. `mppx` keeps this field server-side and doesn't include it in the Challenge.
