...
When a stablecoin payment includes a fractional cent, `mppx` rounds the recorded `PaymentIntent` down to whole cents so its amount doesn't exceed the on-chain transfer.
:::
