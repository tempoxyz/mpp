...
Every `PaymentIntent` that `mppx` creates or records includes `machine_payment: 'true'` by default so you can identify machine payments in Stripe. Matching integration-, method-, or request-scoped metadata overrides the default.
