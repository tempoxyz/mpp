...
Sessions use the `TempoStreamChannel` escrow contract for on-chain deposits, settlement, and channel close. The [IETF Specification](https://paymentauth.org/draft-tempo-session-00) documents the escrow model and voucher format.
