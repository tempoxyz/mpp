...
Use [`tempo`](/sdk/typescript/server/Method.tempo.session) to accept payment sessions. The server needs an RPC URL for on-chain verification during channel open/close, and an atomic store backend for channel state.
...
Use a store that exposes `Store.update()`. `Store.memory()` works for local development, and shared backends need atomic update support.
