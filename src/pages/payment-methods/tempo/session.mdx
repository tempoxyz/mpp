...
| `tempo.session({ account, maxDeposit })` | You want direct lifecycle control with `.fetch()`, `.close()`, `.sse()`, or `.ws()`. Use this when your code must explicitly close a channel. |
...
Use automatic settlement when the server should periodically settle accepted usage while a session remains active. Run settlement from a background job using the highest accepted voucher stored by the server.
...
Any threshold can trigger your settlement job. Use spend amount, metered units, or elapsed time to decide when to call `tempo.settle`.

### Manual settlement

Use manual settlement from an admin workflow, job queue, or close-out process when you want explicit control over timing. `tempo.settle` settles one channel by submitting its highest accepted voucher.
...
const account = privateKeyToAccount('0x0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef')
const store = Session.ChannelStore.fromStore(Store.memory())
...
const txHash = await tempo.settle(store, client, channelId, { account }) // [!code hl]
console.log(txHash)
// @log: 0x...
```

Settle multiple channels from the same job with `Promise.all`.
...
const txHashes = await Promise.all(
  channelIds.map((channelId) => tempo.settle(store, client, channelId, { account })),
) // [!code hl]
...
Register `tempo.session()` when this client should only handle Sessions. Use `tempo.session()` for the standalone lifecycle manager shown in [closing the channel](#closing-the-channel).
...
Use `tempo.session()` when you want direct lifecycle control. Channels remain open for reuse across requests. Call `session.close()` to settle on-chain and reclaim unspent deposit.
...
See [`tempo.session`](/sdk/typescript/client/Method.tempo.session-manager) for the full session lifecycle API.
