...
Set `machineTokenEnabled: true` to let compatible clients pay a single-recipient charge with Tempo's first-party machine token.
...
The server advertises the route in its Challenge. On Tempo mainnet or Moderato, the client uses the route when it holds enough machine tokens and the swap can execute. Otherwise, it falls back to `autoSwap` when configured, then the requested TIP-20 transfer path. The route supports `push`, `pull`, relay, and fee-sponsored settlement; split charges use the standard transfer path.
