...
The server generates a fresh BOLT11 invoice for each request. The client pays it over the Lightning Network via a Spark wallet and presents the payment preimage as a credential. The server verifies `sha256(preimage) == paymentHash` locally and returns the resource with a receipt.
