...
To accept pull mode credentials, provide an `account` so the server can broadcast `receiveWithAuthorization` and pay gas:
...
- **Pull** — the client signs an ERC-3009 `transferWithAuthorization` off-chain. The server broadcasts the authorization on-chain via `receiveWithAuthorization`, paying the gas. This mode requires the server to hold the recipient key.

## Specification

<Cards>
  <SpecCard to="https://paymentauth.org/draft-monad-charge-00" />
</Cards>
