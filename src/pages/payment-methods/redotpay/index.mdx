...
description: "Accept MPP payments using RedotPay balance or crypto rails."
...
# RedotPay [Balance and crypto rails]

The RedotPay payment method enables MPP payments using either a RedotPay balance proof (`rdt`) or a crypto proof (transaction hash), carried inside the MPP Credential payload.
...
<MermaidDiagram chart={`sequenceDiagram
    ...
    Client->>RedotPay: (3) Obtain payment proof (rdt or tx hash)
...
2. **Client** obtains a payment proof from RedotPay rails (balance `rdt` or crypto transaction hash).
