...
The `channel` intent enables high-frequency, pay-as-you-go payments over unidirectional payment channels on Stellar. Clients deposit tokens into an on-chain Soroban escrow contract and sign off-chain cumulative commitments as they consume resources. The server verifies commitments via contract simulation—no per-request on-chain transactions—and closes the channel to settle the final balance.
...
The client deploys a one-way-channel Soroban contract with a commitment key and USDC deposit. This creates a payment channel between the client (funder) and server (recipient). The channel contract holds the deposited SAC tokens on-chain.
...
| `feeBumpSigner` | `Keypair \| string` | — | Fee bump signer for close transactions |
...
| `signer` | `Keypair \| string` | — | Signer for close transactions |
...
Payment channels use the [one-way-channel](https://github.com/stellar-experimental/one-way-channel) Soroban contract for on-chain deposits, commitment verification, and settlement.
