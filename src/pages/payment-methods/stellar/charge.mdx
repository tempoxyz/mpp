...
# Stellar charge [One-time SAC token transfers]

The Stellar implementation of the [charge](/intents/charge) intent.

The client signs a Soroban SAC `transfer` invocation, and the server verifies and broadcasts the transaction on-chain. Settlement completes in ~5 seconds with deterministic finality.

Stellar charge supports two credential modes:

- **Pull mode** (default)—The client signs Soroban auth entries and sends the transaction XDR. The server rebuilds and broadcasts it. Optionally sponsored (server pays fees).
- **Push mode**—The client broadcasts the transaction itself and sends the transaction hash. The server polls for confirmation.
...
<MermaidDiagram chart={`sequenceDiagram
    ...
    Client->>Client: (2) Build SAC transfer, sign auth entries
...
Use `stellar.charge` to gate any endpoint behind a one-time SAC token payment. The method handles Challenge generation, Credential verification, transaction broadcast, and Receipt creation.
...
When `feePayer` is configured, the server adds its own source account and optionally wraps the transaction in a fee bump before broadcasting. The client doesn't need XLM for gas—it signs only the Soroban auth entries.
...
|---|---|---|---|
| `currency` | `string` | — | SAC contract address for the payment token |
...
Use `stellar.charge` with `Mppx.create` to automatically handle `402` responses. The client signs SAC transfer auth entries and retries with the Credential.
...
| `signing` | Building and signing SAC transfer |
