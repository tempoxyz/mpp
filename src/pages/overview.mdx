...
<MermaidAscii chart={`sequenceDiagram
    participant Client
    participant Server
    Client->>Server: (1) GET /resource
    Server-->>Client: (2) 402 + WWW-Authenticate: Payment
    Note over Client: (3) Client fulfills payment
    Client->>Server: (4) GET /resource + Authorization: Payment
    Note over Server: (5) Server verifies payment
    Server-->>Client: (6) 200 OK + Payment-Receipt
`} />
