...
| `agent402-tollbooth` | TypeScript, JavaScript | Pay-per-crawl gate for any Node/Express site: one middleware answers an MPP `WWW-Authenticate: Payment` challenge (evm/charge, tempo/charge) and x402 on the same 402, translates `Authorization: Payment` credentials for the operator's settlement middleware, mirrors `Payment-Receipt`, and adds a proof-of-work free tier, charge modes, and a stats dashboard | [npm](https://www.npmjs.com/package/agent402-tollbooth) · [GitHub](https://github.com/MikeyPetrillo/Agent402/tree/main/tollbooth) |
| `agent402-client` | TypeScript, JavaScript | Buyer SDK for agents: `find()` a paid endpoint, `call()` it, and pay the 402 automatically over MPP (`Authorization: Payment`) or x402 from the agent's own key, with response caching, idempotent retries, and spend caps; non-custodial | [npm](https://www.npmjs.com/package/agent402-client) · [GitHub](https://github.com/MikeyPetrillo/Agent402/tree/main/client) |
