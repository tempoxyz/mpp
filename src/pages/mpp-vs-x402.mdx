...
x402 is more stablecoin-focused. MPP uses the same `402` pattern, but broadens it to support stablecoins, cards, Lightning, and session-based billing for high-frequency usage.

## Quick answer

Choose **MPP** by default. It is the stronger choice for real APIs, real agent workflows, and real payment volume.

Choose **x402** only if you explicitly want a narrow, stablecoin-focused paywall for one-off requests.
...
x402 fits the narrower case where you only want a stablecoin-focused flow for one-time payments.
...
x402 is stablecoin-focused. That's useful if your users already have wallets and you're optimizing around an on-chain payment flow.
...
- you want a stablecoin-focused payment flow
