...
:::tip
If your users hold a different stablecoin than the one the server requests, enable [auto-swap](/payment-methods/tempo#auto-swap) to automatically convert via the Tempo DEX before paying:

```ts
Mppx.create({
  methods: [tempo.charge({ account, autoSwap: true })],
})
```
:::
