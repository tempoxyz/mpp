import { Badge, Card, Cards } from 'vocs'

# Use with your agent [Interact with MPP enabled services with your agent of choice]

Agents can automatically interact with MPP-enabled services. Here's how to get started.

## Tempo Wallet <Badge variant="info">Recommended</Badge>

The [Tempo Wallet](https://wallet.tempo.xyz) is a managed MPP client with built in spend controls and service discovery. Agents can use tempo wallet to seamlessly pay for powerful new capabilities on demand. 

### Automatic setup 

Paste this into your agent to set up Tempo Wallet:

::::code-group

```bash [Amp]
echo "Read and follow https://wallet.tempo.xyz/skill.md" | amp
```

```bash [Claude]
claude "Read and follow https://wallet.tempo.xyz/skill.md"
```

```bash [Codex]
codex "Read and follow https://wallet.tempo.xyz/skill.md"
```

```bash [Other]
# Prompt your agent
> Read and follow https://wallet.tempo.xyz/skill.md
```

::::

### Manual setup

```bash
# Install the CLI
curl -fsSL https://cli.tempo.xyz/install | bash

# Connect your wallet 
tempo wallet login

# Verify setup
tempo wallet whoami
```

Once setup, use Tempo Wallet to discover and call services:

**List available services**

```bash
tempo mpp services
```

**Make a paid request**

```bash
tempo mpp -X POST \
  --json '{"prompt": "a sunset over the ocean"}' \
  https://fal.mpp.tempo.xyz/fal-ai/flux/dev
```

## mppx

The [`mppx`](/sdk/typescript/cli) CLI is a lightweight MPP client bundled with the `mppx` package. It is designed for simple use cases and debugging during development.

### Install

:::code-group
```bash [npm]
npm install -g mppx
```
```bash [pnpm]
pnpm add -g mppx
```
```bash [bun]
bun add -g mppx
```
:::

### Get started

```bash
# Create an account
mppx account create

# Make a paid request
mppx https://mpp.dev/api/ping/paid
```

## Next steps

<Cards>
  <Card
    description="Browse available MPP services and their endpoints"
    icon="lucide:list"
    title="Services"
    to="/services"
  />
</Cards>
