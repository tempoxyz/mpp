...
$ presto sessions close https://openrouter.mpp.tempo.xyz
...
# Silent mode (suppress logs, recommended for agents)
$ presto -s -X POST \
  --json '{"model":"openai/gpt-4o-mini","messages":[{"role":"user","content":"Hello"}]}' \
  https://openrouter.mpp.tempo.xyz/v1/chat/completions
...
# TOON output (compact, token-efficient — recommended for agents)
$ presto -t -X POST \
  --json '{"model":"openai/gpt-4o-mini","messages":[{"role":"user","content":"Hello"}]}' \
  https://openrouter.mpp.tempo.xyz/v1/chat/completions
