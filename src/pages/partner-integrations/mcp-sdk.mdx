...
Choose a local viem account for development or use a Privy server wallet when the MCP client runs in production.
...
Create an EVM server wallet in [Privy](https://dashboard.privy.io), fund its address with pathUSD on Tempo, then store its ID and address with your Privy app credentials. Keep `PRIVY_APP_SECRET` server-side.
