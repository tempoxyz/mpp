...
When an HTTP handler composes multiple offers, `mppx` excludes Stripe offers below Stripe's general minimum for known currencies before it calls your `canOffer` hook.

### canOffer (optional)

- **Type:** `Method.CanOfferFn`

Returns whether this configured Stripe offer is available when an HTTP handler composes multiple offers. The hook receives the normalized, immutable payment request and a clone of the incoming `Request`.
