...
Pass a deposit-address resolver, then use `defaultMethods()` to offer Tempo stablecoins and Stripe Shared Payment Tokens. `mppx` records successful Tempo charges and eligible Session settlements as Stripe `PaymentIntent` objects.
...
### Record Tempo Session settlements

Add a Tempo Session method through `defaultMethods().additional()`. Each settlement records the newly settled amount as a Stripe `PaymentIntent`, rounded down to whole cents, before `mppx` calls your optional `onSessionSettlement` hook. `mppx` skips recording when the amount rounds down below one cent.
...
const mppx = Mppx.create({
  methods: await payments.defaultMethods().additional({
    tempo: { session: {} },
  }),
  secretKey: process.env.MPP_SECRET_KEY!,
})
...
The recorded `PaymentIntent` uses the settlement transaction hash for idempotency and includes `mpp_intent: 'session'` metadata. Session settlements don't include `mpp_challenge_id` because the settlement callback doesn't receive a Challenge.
...
For stablecoin payments with a fractional-cent amount, `mppx` rounds the recorded `PaymentIntent` down to whole cents so it never records more than the on-chain transfer.
...
Call `.additional(config)` before awaiting asynchronous address resolution. The config accepts `base`, `solana`, and `tempo.session` entries. Stablecoin charge methods record successful payments in Stripe without replacing their existing `onPaymentSuccess` hooks. Tempo Session methods record settlement deltas that floor to at least one cent before calling their existing `onSessionSettlement` hooks.
...
Creates a Tempo session method for a Stripe deposit address. Pass the remaining [`tempo.session`](/sdk/typescript/server/Method.tempo.session) configuration. Each settlement that floors to at least one cent records the newly settled amount as a Stripe `PaymentIntent` before the optional `onSessionSettlement` hook runs.
...
Key-value pairs attached to Stripe `PaymentIntent` objects created or recorded by SPT, stablecoin charge, and Tempo Session methods. Per-method `metadata` on `base.charge()` or `tempo.charge()` overrides matching keys. `mppx` adds `machine_payment`, `mpp_intent`, and `mpp_sdk` analytics keys by default. Charge payments also include `mpp_challenge_id`; Session settlements don't. Matching integration-, method-, or request-scoped metadata overrides built-in values.
