...
Use `Store.memory()` for local development, tests, or a single long-lived server process. Use `Store.redis()`, `Store.upstash()`, or `Store.cloudflare()` when you need replay prevention to survive restarts or apply across multiple instances.
