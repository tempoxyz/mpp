...
Pass `store` when you want zero-dollar proof Credentials to be single-use. The store must support atomic updates.
...
Pass an atomic store when you want replay protection for charge Credentials.
...
Use `Store.memory()` for local development, tests, or a single long-lived server process. For shared deployments, configure `Store.redis()`, `Store.upstash()`, or `Store.cloudflare()` with atomic `update` support so they return `Store.AtomicStore`.

`mppx` uses `Store.update()` to write replay markers atomically. Keep `update` callbacks synchronous and side-effect free because adapters may retry them.
