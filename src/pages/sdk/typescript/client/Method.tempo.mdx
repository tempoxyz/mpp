...
Use this helper when you want `Mppx.create` to handle both one-time charges and current Sessions through the same fetch wrapper. If you need to close or stream a single channel explicitly, use [`tempo.session()`](/sdk/typescript/client/Method.tempo.session-manager) instead.
...
`tempo.session()` creates a standalone Sessions manager with explicit `.fetch()`, `.close()`, `.sse()`, and `.ws()` methods. It does not register a method in `Mppx.create`; it owns one channel lifecycle directly.
...
See [`tempo.session`](/sdk/typescript/client/Method.tempo.session-manager) for the full API.
