...
const channels = new Map<string, Parameters<Session.Client.ChannelStore['set']>[0]>()
const channelStore: Session.Client.ChannelStore = {
  delete(key) {
    channels.delete(key)
  },
  get(key) {
  ...
  set(channel) {
    channels.set(Session.Client.entryKey(channel), channel)
  },
}
...
const storagePrefix = 'mppx-session:api.example.com:user-123'

const channelStore = Session.Client.createJsonChannelStore({
  delete(key) {
...
type ChannelStore = {
  ...
  set(channel: ChannelEntry): Promise<void> | void
}
```
