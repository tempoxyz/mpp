...
type SessionRequestInit = RequestInit & {
  orderChallenges?: SessionOrderChallenges
}
...
WebSocket constructor for runtimes without a global `WebSocket` (for example Node.js). Required for `session.ws()` in non-browser environments. Use [`isows`](https://github.com/wevm/isows) for isomorphic support.
