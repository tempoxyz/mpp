...
Register event handlers to log or trace the automatic `402` flow. Typed helpers cover common events, event constants map to the lifecycle event names, and `*` receives every client event.
...
# Observe the selected Challenge before the client creates a Credential.
...
# Observe the retried response after payment.
...
`on_challenge_received` can return a `Credential` or `Payment` authorization string to override the default Credential creation path. Other handlers only observe payment handling. Each registration returns an unsubscribe proc.
