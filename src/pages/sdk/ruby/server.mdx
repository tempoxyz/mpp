...
Register event handlers to monitor issued Challenges, successful payments, and rejected Credentials. Typed helpers cover common events, event constants map to the lifecycle event names, and `*` receives every server event.
...
# Observe each Challenge issued by your API.
