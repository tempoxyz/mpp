...
Register event handlers to observe successful Credential verification. Use the typed helper for `payment.success`, `on()` for a specific event kind, or `on_any()` to receive every server event.
...
// Observe successful Credential verification.
...
Server handlers run inline with verification. Keep the returned subscription handles alive while handlers should remain registered.
