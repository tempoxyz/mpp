...
Create an `Mpp` instance with `Mpp::create()` and call `charge()` with a human-readable dollar amount. The `tempo()` factory creates a Tempo payment method—configure `recipient` once, then every `charge()` call uses those defaults.
...
use mpp::server::{Mpp, tempo, TempoConfig};
use mpp::{PaymentChallenge, parse_authorization, format_www_authenticate};
...
async fn get_resource(
    ...
    match auth {
        Some(auth_header) => {
            ...
            (StatusCode::OK, Json(serde_json::json!({ "paid": true }))).into_response()
        }
        None => {
            let challenge = mpp.charge("0.50").unwrap();
            let header = format_www_authenticate(&challenge).unwrap();
            (StatusCode::PAYMENT_REQUIRED, [("WWW-Authenticate", header)]).into_response()
        }
    }
}
```
