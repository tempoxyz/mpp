...
Add `.send_with_payment()` to any `reqwest::RequestBuilder`. The extension trait handles the challenge→credential→retry flow for you.
...
```rust
use mpp::client::{Fetch, MultiProvider, TempoProvider};
