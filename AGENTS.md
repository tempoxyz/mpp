...
10. **Spec link Cards** - Use the shared `<SpecCard to="..." />` component. Defaults to title `"Specification"` and description `"Read the full specification"`. Override with `title` and `description` props when linking to a specific draft.
11. **"Specification"** - Use "Specification" when referring to the specifications collectively, not "Specifications"
